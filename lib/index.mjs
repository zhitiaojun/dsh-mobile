...
async function regularFile$3(path, maximum, field) {
	let info;
	try {
...
async function containedPath(root, relativePath, maximum, field) {
	...
	return regularFile$3(targetReal, maximum, field);
}
...
async function extensionFingerprint(directory) {
	...
	const manifestFile = await regularFile$3(join(root, "extension.json"), EXTENSION_LIMITS.manifest, "extension.json");
...
const GITHUB_HOSTS$1 = ["github.com", "githubusercontent.com"];
...
const releases = [
	{
		...
		allowedDownloadHosts: GITHUB_HOSTS$1,
...
async function regularFile$2(file) {
	try {
...
async function runCapture$1(file, args) {
...
async function defaultExtractArtifact$1(archive, destination, executableName) {
	...
	const executableEntry = selectFrpExecutableEntry((await runCapture$1(tar, ["-tf", archive])).split(/\r?\n/u).filter((entry) => entry.length > 0), executableName);
	const unpacked = join(destination, "archive");
	await mkdir(unpacked, {
		recursive: true,
		mode: 448
	});
	await runCapture$1(tar, [
		"-xf",
		archive,
		"-C",
		unpacked,
	...
	if (!await regularFile$2(extracted)) throw new Error("frp_archive_executable_invalid");
...
async function rootExtractArtifact(archive, destination, executableName) {
	const tar = process.platform === "win32" ? "tar.exe" : "tar";
	const executableEntry = selectRootExecutableEntry((await runCapture$1(tar, ["-tf", archive])).split(/\r?\n/u).filter((entry) => entry.length > 0), executableName);
	const unpacked = join(destination, "archive");
	await mkdir(unpacked, {
		recursive: true,
		mode: 448
...
function hostAllowed$1(hostname, allowed) {
	return allowed.some((host) => hostname === host || hostname.endsWith(`.${host}`));
}
async function defaultFetchArtifact$2(artifact, signal) {
	...
	if (finalUrl.protocol !== "https:" || !hostAllowed$1(finalUrl.hostname, artifact.allowedDownloadHosts)) throw new Error("frp_download_origin_invalid");
...
async function defaultInspectExecutable$1(executable) {
	return (await runCapture$1(executable, ["--version"])).trim();
}
...
var FrpComponentManager = class {
	...
	constructor(options) {
		...
		for (const child of [
			...
			this.logRoot,
			this.stagingRoot
		]) if (!inside$2(stateDirectory, child)) throw new Error("frp component path escaped its state directory");
		this.fetchArtifact = options.fetchArtifact ?? defaultFetchArtifact$2;
		...
		this.inspectExecutable = options.inspectExecutable ?? defaultInspectExecutable$1;
	}
	/** Inspect the managed executable without relying on global FRP installations. */
	async initialize() {
		this.installed = await regularFile$2(this.executable);
	...
	install() {
		return this.enqueue(async () => {
			...
			try {
				...
				if (sha256$2(bytes) !== artifact.downloadSha256) throw new Error("frp_download_hash_mismatch");
				...
				if (!await regularFile$2(extracted)) throw new Error("frp_executable_missing");
...
//#endregion
//#region src/cloudflared-component.ts
...
async function defaultFetchArtifact$1(artifact, signal) {
	...
	const finalUrl = new URL(response.url);
	if (finalUrl.protocol !== "https:" || !hostAllowed(finalUrl.hostname, artifact.allowedDownloadHosts)) throw new Error("cloudflared_download_origin_invalid");
	const declaredLength = Number(response.headers.get("content-length"));
	if (Number.isFinite(declaredLength) && declaredLength !== 0 && declaredLength !== artifact.downloadBytes) throw new Error("cloudflared_download_size_mismatch");
...
var CloudflaredComponentManager = class {
	executable;
	componentRoot;
	logRoot;
	componentStorage;
	...
	constructor(options) {
		...
		for (const child of [
			...
			this.logRoot,
			this.stagingRoot
		]) if (!inside$1(stateDirectory, child)) throw new Error("cloudflared component path escaped its state directory");
	...
	install() {
		return this.enqueue(async () => {
			const artifact = this.artifact;
			if (artifact === void 0) throw new Error("cloudflared_component_unsupported");
			await mkdir(this.stagingRoot, {
				recursive: true,
				mode: 448
			});
			...
			try {
				...
				if (sha256$1(bytes) !== artifact.downloadSha256) throw new Error("cloudflared_download_hash_mismatch");
				const candidate = join(staging, artifact.executableName);
				await writeFile(candidate, bytes, {
					flag: "wx",
					mode: 448
				});
				await chmod(candidate, 448);
	...
	async publish(candidate) {
		await mkdir(this.componentStorage, {
			recursive: true,
			mode: 448
		});
		...
		try {
			...
			await chmod(target, 448).catch(() => void 0);
		}
	}
...
function isProviderChoice(value) {
	return value === "tailscale" || value === "cpolar" || value === "frp" || value === "chmlfrp" || value === "cloudflare";
}
...
*/
const START_TIMEOUT_MS$2 = 15e4;
const DISCOVERY_REQUEST_TIMEOUT_MS$1 = 5e3;
/**
...
* just this panel.
*/
const DISCOVERY_RETRY_MS$1 = 1500;
const DISCOVERY_RETRY_MAX_MS = 15e3;
const MAX_DISCOVERY_BYTES$1 = 16384;
...
function publicStatus$3(status) {
	return Object.freeze({
...
function parseQuickTunnelOrigin(line) {
	const hostname = QUICK_TUNNEL_URL.exec(line)?.[1];
	return hostname === void 0 ? void 0 : `https://${hostname.toLowerCase()}`;
}
function defaultLaunchClient$1(executable, listenPort) {
	return spawn(executable, [
		"tunnel",
		"--no-autoupdate",
		"--url",
		`http://127.0.0.1:${String(listenPort)}`
	], {
		shell: false,
		windowsHide: true,
		stdio: [
...
async function boundedResponseBytes$1(response) {
	if (response.body === null) throw new Error("cloudflare_discovery_invalid");
	const declaredLength = Number(response.headers.get("content-length"));
	if (Number.isFinite(declaredLength) && declaredLength > MAX_DISCOVERY_BYTES$1) throw new Error("cloudflare_discovery_invalid");
	const reader = response.body.getReader();
	const chunks = [];
	let received = 0;
	while (true) {
		...
		if (result.done) break;
		received += result.value.byteLength;
		if (received > MAX_DISCOVERY_BYTES$1) {
			await reader.cancel();
...
async function defaultProbeDiscovery$1(origin, expectedInstanceId, signal) {
	const controller = new AbortController();
	const abort = () => {
		controller.abort();
	};
	signal.addEventListener("abort", abort, { once: true });
	const timeout = setTimeout(abort, DISCOVERY_REQUEST_TIMEOUT_MS$1);
	timeout.unref();
	try {
		...
		let value;
		try {
			value = JSON.parse(new TextDecoder().decode(await boundedResponseBytes$1(response)));
		} catch {
			throw new Error("cloudflare_discovery_invalid");
		}
...
*/
var CloudflareController = class {
	options;
	enabled = false;
	initialized = false;
	disposed = false;
	...
	status() {
		return publicStatus$3(this.latest);
	}
	...
	publish(status) {
		this.latest = publicStatus$3(status);
		try {
	...
	async start() {
		...
		try {
			...
			this.publish({
				enabled: true,
				state: "unavailable",
				errorCode: "cloudflare_component_missing"
			});
			return;
		}
		if (!entry.isFile() || entry.isSymbolicLink()) {
			this.publish({
				enabled: true,
				state: "unavailable",
				errorCode: "cloudflare_component_invalid"
			});
			return;
		}
		this.publish({
		...
		let child;
		try {
			child = (this.options.launchClient ?? defaultLaunchClient$1)(this.options.executable, listenPort);
		} catch {
			this.publish({
				enabled: true,
				state: "error",
				errorCode: "cloudflare_launch_failed"
			});
			return;
		}
		this.child = child;
		const origin = await new Promise((resolveOrigin) => {
			...
			child.once("close", () => finish(void 0));
			const timer = setTimeout(() => finish(void 0), this.options.startTimeoutMs ?? START_TIMEOUT_MS$2);
			timer.unref();
		});
		if (generation !== this.generation || !this.enabled) return;
		if (origin === void 0) {
		...
		child.once("error", () => {
			this.enqueue(() => this.failGeneration(generation, "cloudflare_launch_failed"));
		});
		child.once("close", (code) => {
			if (generation !== this.generation || this.child !== child) return;
			this.child = void 0;
			if (this.enabled) this.enqueue(() => this.failGeneration(generation, code === 0 ? "cloudflare_stopped" : "cloudflare_exited"));
		});
		this.publish({
			enabled: true,
			state: "connecting",
		...
		this.waitForDiscovery(generation, origin, controller.signal);
	}
	async waitForDiscovery(generation, origin, signal) {
		const deadline = Date.now() + (this.options.startTimeoutMs ?? START_TIMEOUT_MS$2);
		const probe = this.options.probeDiscovery ?? defaultProbeDiscovery$1;
		const instanceId = this.options.instanceId;
		let lastFailure;
		let delay = this.options.retryIntervalMs ?? DISCOVERY_RETRY_MS$1;
	...
	async stopProcessAndGateway() {
		...
		await settleRemoteResources([() => child !== void 0 && child.exitCode === null ? terminateRemoteProcess(child) : void 0, () => gateway?.close()], "Cloudflare resource cleanup failed");
	}
};
//#endregion
...
function remoteControlPayload(provider, status, gateway, providerStatuses, cpolarComponent, frpComponent, frpConfiguration, chmlfrpComponent, cloudflaredComponent) {
	return {
		provider,
...
async function apply(ctx, config) {
	...
	const remoteControllers = {
		...
		cloudflare: new CloudflareController({
			...
			createGateway: createRemoteGateway,
			onStatus(status) {
				logger.info("cloudflare state=%s origin=%s%s", status.state, status.origin ?? "none", status.errorCode === void 0 ? "" : ` error=${status.errorCode}`);
			},
	...
	}, cpolarComponent.status(), frpComponent.status(), frpConfig.status(), chmlfrpComponent.status(), cloudflaredComponent.status());
	const lanPayload = () => ({
		...
		handler: async (request, response) => {
			try {
				...
				if (request.method === "POST" && target.decodedPathname === `/api/mobile-access/remote/provider`) {
					const body = await readJsonObject(request, 4096);
					if (body.provider !== "tailscale" && body.provider !== "cpolar" && body.provider !== "frp" && body.provider !== "chmlfrp" && body.provider !== "cloudflare") throw new HttpError(400, "bad_request");
				...
				if (request.method === "POST" && target.decodedPathname === `/api/mobile-access/remote/cloudflare/component/install`) {
					if ((await readJsonObject(request, 4096)).confirm !== true) throw new HttpError(400, "bad_request");
				...
				if (request.method === "POST" && target.decodedPathname === `/api/mobile-access/remote/cloudflare/component/purge`) {
					if ((await readJsonObject(request, 4096)).confirm !== true) throw new HttpError(400, "bad_request");
					await remoteProviders.mutate(async () => {
