window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		Object.assign(MOBILE_CONTROL_MESSAGES.en, {
			...
			cloudflareName: "Cloudflare Tunnel",
			cloudflareDescription: "A free quick tunnel: no account, no own domain, no VPS. The public address is assigned at runtime and changes on every restart.",
			cloudflareInstallClient: "Install cloudflared",
			cloudflareInstallConfirm: "Download and verify cloudflared {version} (about {size} MB) from the official Cloudflare release? Only the binary is extracted into the DSH Mobile private directory.",
			cloudflareClientReady: "cloudflared {version} is verified.",
			cloudflareClientMissing: "cloudflared is not installed. Nothing is downloaded until you click Install.",
			cloudflareQuickNote: "Quick tunnels are public URLs. Anyone with the address can reach the gateway; only paired devices can get in. Do not share it.",
			cloudflarePurge: "Remove cloudflared and its configuration",
			cloudflarePurgeConfirm: "Stop the tunnel and remove the cloudflared binary, staging files and logs owned by DSH Mobile?",
			noAccountNeeded: "No account needed",
			switchingCloudflare: "Switching to the Cloudflare tunnel…",
		...
		Object.assign(MOBILE_CONTROL_MESSAGES.it, {
			...
			cloudflareName: "Cloudflare Tunnel",
			cloudflareDescription: "Un quick tunnel gratuito: nessun account, nessun dominio, nessun VPS. L’indirizzo pubblico viene assegnato a runtime e cambia a ogni riavvio.",
			cloudflareInstallClient: "Installa cloudflared",
			cloudflareInstallConfirm: "Scaricare e verificare cloudflared {version} (circa {size} MB) dalla release ufficiale Cloudflare? Solo il binario viene estratto nella directory privata DSH Mobile.",
			cloudflareClientReady: "cloudflared {version} verificato.",
			cloudflareClientMissing: "cloudflared non è installato. Nulla viene scaricato finché non scegli Installa.",
			cloudflareQuickNote: "I quick tunnel sono URL pubblici. Chiunque abbia l’indirizzo può raggiungere il gateway; solo i dispositivi associati entrano. Non condividerlo.",
			cloudflarePurge: "Rimuovi cloudflared e la sua configurazione",
			cloudflarePurgeConfirm: "Arrestare il tunnel e rimuovere il binario cloudflared, i file temporanei e i log gestiti da DSH Mobile?",
			noAccountNeeded: "Nessun account richiesto",
			switchingCloudflare: "Passaggio al tunnel Cloudflare…",
		...
		Object.assign(MOBILE_CONTROL_MESSAGES.zh, {
			...
			cloudflareName: "Cloudflare 隧道",
			cloudflareDescription: "免费临时隧道：无需账号、无需自有域名、无需 VPS。公网地址在运行时分配，每次重启都会变。",
			cloudflareInstallClient: "安装 cloudflared",
			cloudflareInstallConfirm: "从 Cloudflare 官方 Release 下载并校验 cloudflared {version}（约 {size} MB）？只提取二进制到 DSH Mobile 私有目录。",
			cloudflareClientReady: "cloudflared {version} 已通过校验。",
			cloudflareClientMissing: "cloudflared 尚未安装；点击下方按钮后才会开始下载。",
			cloudflareQuickNote: "临时隧道是公开地址，拿到链接的人都能访问网关；只有已配对设备能进入。请不要对外分享。",
			cloudflarePurge: "移除 cloudflared 与其配置",
			cloudflarePurgeConfirm: "关闭隧道并移除 DSH Mobile 管理的 cloudflared 二进制、临时文件和日志？",
			noAccountNeeded: "无需账号",
			switchingCloudflare: "正在切换到 Cloudflare 隧道…",
			prepareFrp: "配置自建 FRP",
...
`;
		...
		*/
		...
		function installControl() {
			...
			const cloudflareChoice = element("button", "dsh-mobile-control__provider");
			cloudflareChoice.type = "button";
			cloudflareChoice.setAttribute("role", "radio");
			cloudflareChoice.setAttribute("aria-checked", "false");
			const cloudflareChoiceTop = element("span", "dsh-mobile-control__provider-top");
			const cloudflareChoiceName = element("strong");
			cloudflareChoiceName.textContent = t("cloudflareName");
			const cloudflareChoiceBadge = element("span", "dsh-mobile-control__provider-badge");
			cloudflareChoiceBadge.textContent = t("noAccountNeeded");
			const cloudflareChoiceDescription = element("span", "dsh-mobile-control__provider-description");
			cloudflareChoiceDescription.textContent = t("cloudflareDescription");
			cloudflareChoiceTop.append(cloudflareChoiceName, cloudflareChoiceBadge);
			cloudflareChoice.append(cloudflareChoiceTop, cloudflareChoiceDescription);
			...
			const cloudflareSetup = element("section", "dsh-mobile-control__cpolar-setup");
			cloudflareSetup.hidden = true;
			const cloudflareSetupTitle = element("h3", "dsh-mobile-control__section-title");
			cloudflareSetupTitle.textContent = t("cloudflareName");
			const cloudflareComponentStatus = element("p", "dsh-mobile-control__component-status");
			cloudflareComponentStatus.textContent = t("checkingComponent");
			const cloudflareInstall = element("button", "dsh-mobile-control__primary dsh-mobile-control__frp-action");
			cloudflareInstall.type = "button";
			cloudflareInstall.textContent = t("cloudflareInstallClient");
			const cloudflareNote = element("p", "dsh-mobile-control__frp-requirement");
			cloudflareNote.textContent = t("cloudflareQuickNote");
			const cloudflareStatus = element("p", "dsh-mobile-control__component-status");
			cloudflareStatus.textContent = t("frpConfigurationMissing");
			const cloudflareOrigin = element("code", "dsh-mobile-control__storage");
			cloudflareOrigin.textContent = "";
			const cloudflareConnect = element("button", "dsh-mobile-control__primary dsh-mobile-control__frp-action");
			cloudflareConnect.type = "button";
			cloudflareConnect.textContent = t("saveConnect");
			const cloudflarePurge = element("button", "dsh-mobile-control__danger");
			cloudflarePurge.type = "button";
			cloudflarePurge.textContent = t("cloudflarePurge");
			...
			providerSetupBody.append(cpolarSetup, frpSetup, cloudflareSetup, tailscaleInfo);
			...
			const renderRemote = (data) => {
				...
				remoteProvider = data.provider === "cpolar" ? "cpolar" : data.provider === "frp" ? "frp" : data.provider === "cloudflare" ? "cloudflare" : "tailscale";
				const cpolar = remoteProvider === "cpolar";
				const frp = remoteProvider === "frp";
				const tailscale = remoteProvider === "tailscale";
				const cloudflare = remoteProvider === "cloudflare";
				...
				cloudflareChoice.classList.toggle("is-selected", cloudflare);
				...
				cloudflareChoice.setAttribute("aria-checked", String(cloudflare));
				providerSetupName.textContent = cpolar ? "cpolar" : frp ? t("frpName") : cloudflare ? t("cloudflareName") : "Tailscale Funnel";
				tailscaleChoice.disabled = remoteProviderBusy;
				cpolarChoice.disabled = remoteProviderBusy;
				frpChoice.disabled = remoteProviderBusy;
				cloudflareChoice.disabled = remoteProviderBusy;
				...
				chmlfrpSummary.append(chmlfrpSummaryText, chmlfrpBadge);
				const cloudflareProvider = providers.cloudflare !== null && typeof providers.cloudflare === "object" ? providers.cloudflare : {};
				const cloudflareComponent = cloudflareProvider.component !== null && typeof cloudflareProvider.component === "object" ? cloudflareProvider.component : {};
				...
				cloudflareInstall.textContent = cloudflareBytes > 0 ? t("installWithSize", { size: formatMegabytes(cloudflareBytes) }) : t("cloudflareInstallClient");
				...
				cloudflareComponentStatus.textContent = !cloudflareSupported ? t("frpUnsupported") : cloudflareInstalled ? t("cloudflareClientReady", { version: cloudflareVersion }) : t("cloudflareClientMissing");
				cloudflareStatus.textContent = !cloudflareRunning ? t("remoteStateOff") : cloudflareProvider.state === "ready" ? t("ready") : cloudflareProvider.state === "starting" || cloudflareProvider.state === "connecting" ? t("remoteStateConnecting") : cloudflareProvider.state === "error" || cloudflareProvider.state === "unavailable" ? t("remoteStateAttention") : t("remoteStateOff");
			...
			cloudflareChoice.addEventListener("click", () => {
				chooseRemoteProvider("cloudflare");
			});
			...
			*/
			...
			cloudflareInstall.addEventListener("click", () => {
				if (remoteProviderBusy) return;
				if (!window.confirm(t("cloudflareInstallConfirm", {
					version: cloudflareClientVersion,
					size: cloudflareDownloadSize
				}))) return;
				remoteProviderBusy = true;
				cloudflareInstall.disabled = true;
				cloudflareInstall.textContent = t("downloading");
				remoteStatus.textContent = t("installingFrp");
				controlRequestJson("/api/mobile-access/remote/cloudflare/component/install", {
					method: "POST",
					body: JSON.stringify({ confirm: true })
				}, LONG_CONTROL_REQUEST_TIMEOUT_MS).then(renderRemote, (error) => {
					remoteStatus.textContent = t("installFailed", { error: String(error) });
			...
			cloudflareConnect.addEventListener("click", () => {
				...
				}).then(() => controlRequestJson("/api/mobile-access/remote/control", {
					method: "POST",
					body: JSON.stringify({ running: true })
				})).then(renderRemote, (error) => {
					remoteStatus.textContent = t("configureFailed", { error: String(error) });
			...
			cloudflarePurge.addEventListener("click", () => {
				...
				remoteStatus.textContent = t("purgingFrp");
				controlRequestJson("/api/mobile-access/remote/cloudflare/component/purge", {
					method: "POST",
					body: JSON.stringify({ confirm: true })
				}, LONG_CONTROL_REQUEST_TIMEOUT_MS).then(renderRemote, (error) => {
					remoteStatus.textContent = t("purgeFailed", { error: String(error) });
					...
					remoteProviderBusy = false;
					cloudflarePurge.disabled = false;
					loadRemote();
				});
			});
